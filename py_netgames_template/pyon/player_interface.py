...
class PlayerInterface(PyNetgamesServerListener):
    ...
    def update_gui_status(self,winner):
        ...
        if(new_status == 5 and self.local_enabled==True):
            new_message ="Movimento invalida"
    ...
    def setup_window(self):
        ...
        self.menu_file.add_command(label="Desconectar", command=lambda: print("desconectar"), state="disabled")
    ...
    def select_board_place(self, event, x, y):
        ...
        if (self.local_enabled and self.is_piece_selected()):
            ...
            if (dict["valid_move"]):
                ...
                self.server_proxy.send_move(self.match_id,{"positions": self.tabuleiro.get_all_owners(),"turno":dict["troca_turno"]})
                self.update_gui_status(self.tabuleiro.evaluate_winner())
                self.arruma_interface()
        ...
        elif(self.local_enabled and self.tabuleiro.get_position(x,y).my_owner()==self.tabuleiro.local_player_team() and self.tabuleiro.get_status()==3):
            self.selected = self.tabuleiro.get_position(x,y)
            self.show_possible_moves(x,y) 
            self.update_gui_status(self.tabuleiro.evaluate_winner())
            self.arruma_interface()
        elif(self.local_enabled and self.tabuleiro.get_position(x,y).get_captured() and self.tabuleiro.get_status()==2):
            self.selected = self.tabuleiro.get_position(x,y)

...
class Board:
    def __init__(self):
        ...
        for x in range(5):
            for y in range(3):
                if(x == 0):#base branca
                    self.positions[x][y]=Position(x,y,1,1)
                elif(x==4):#base vermelha
                    self.positions[x][y]=Position(x,y,0,0)
                else:#demais posicoes
                    self.positions[x][y]=Position(x,y,None,2)
        ...
        for x in range(5):
            for y in range(3):
                if(x == 0):#base branca
                    ...
                    self.get_position(x,y).set_owner(None)
        self.match_status = 3
    ...
    def Is_opponent_piece(self,x, y):
        ...
        if (position.owner != self.local_player.get_team() and position.owner != 2):
